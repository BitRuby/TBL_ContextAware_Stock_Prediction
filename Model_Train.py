...
TOKENIZER_NAME = "ElKulako/cryptobert" #"ElKulako/cryptobert" # ProsusAI/finbert # deepseek-ai/DeepSeek-R1
...
class Model_Train():
    ...
    def predict(self, model, tokenizer, prompts):
        """
        Classify the given data.
 
        Args:
        model (AutoModelForSequenceClassification): A pretrained loaded model.
        tokenizer (PreTrainedTokenizer): A HuggingFace tokenizer object used to tokenize the text data.
        prompts (string[]): A list of prompts.
 
        Returns:
        Tensor[List]: Predictions
        """
        encoded = tokenizer(prompts, padding=True, truncation=True, return_tensors="pt", max_length=512)
        with torch.no_grad():
    ...
    def start(self, dataframe_with_day: pd.DataFrame, hf_dataset: 'datasets.Dataset', n_splits = 5, num_epochs = 2):
        ...
        for index, _ in tqdm(enumerate(train_folds), desc="Fold Progress..."):
            ...
            model = AutoModelForSequenceClassification.from_pretrained(TOKENIZER_NAME, config=config, ignore_mismatched_sizes=True)
